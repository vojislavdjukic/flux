...
int FastpassFlow::remaining_flow_size_estimate(){
	return this->sender_remaining_num_pkts;
}

void FastpassFlow::update_remaining_size() {
	int r = remaining_flow_size_estimate();
    FastpassRTS* rts = new FastpassRTS(this, this->src, dynamic_cast<FastpassTopology*>(topology)->arbiter, this->sender_finished?-1:r);
